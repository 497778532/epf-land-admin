<template>
    <div>
        <el-container class="el-container-dialog">
            ...
            <el-main>
                <el-form :model="singleuser" :rules="validations" ref="validations" label-width="160px" class="demo-ruleForm">
                         ...
                         <el-form-item label='所属机构' prop="orgName" >
                           <el-select v-model="mineOrg" placeholder="请选择隶属区域" multiple collapse-tags   style="width:300px;"  >
                             <el-option :value="mineOrg" style="height: auto">
                               <!-- default-expand-all  这是默认展开所有树数据-->
                               <el-tree :data="orgCascaderOptions" show-checkbox check-strictly node-key="id"
        ...
        <div align="middle">
            <el-button v-bind:disabled="userAdd.display" type="primary" @click="addUser">保存</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            initialUserObject() {
                this.singleuser["orgId"] = this.modelParam["orgId"];
                this.$post(this.$API.EPF_ADMIN.USER.INIT,{userId: '123'}).then(r => {
                    ...
                    this.defaultCode=r.data.regionData[0].id;//设置默认值
                });
            },
            ...
            addUser() {
                let _this = this;
                if(_this.singleuser.regional==null||_this.singleuser.regional==""){
                    _this.$message.error("请选择隶属区域!");
                    return;
                }
                this.$refs['validations'].validate((valid) => {
                    if (valid) {
                       this.$post(_this.$API.EPF_ADMIN.USER.ADD, _this.singleuser).then(res => {
          ...
          regionCheckChange(data, checked, node) {
              ...
              if (checked) {//加载机构树数据
                ...
                this.singleuser["regionalsId"] =data.regionCode;
                ...
                if(getregionCode!=""){
                  this.$post(this.$API.EPF_ADMIN.ORG.OrgTree,{regionalsId: getregionCode}).then(r => {
                    debugger
                    this.orgCascaderOptions=r.data.ztreeJson;
          ...
          orgIdsSelected(data, checked, node) {
            ...
            getImgUrl(id) {
                return "/api"+this.$API.EPF_DOCUMENT.IMAGESRC + id;
            }
        }
    }
</script>
