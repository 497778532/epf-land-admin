<template>
  <epf-container title="服务鉴权配置" id="deployCenter">
      ...
      <el-main>
        <template>
          <el-table :data="tableDataList"
            ...
            <el-table-column fixed prop="name" label="配置名" min-width="110">
              <template slot-scope="scope">
            ...
            <el-table-column fixed prop="value1" label="值" min-width="110">
              <template slot-scope="scope">
                  <span>{{scope.row.value1}}</span>
              </template>
            </el-table-column>

            <el-table-column fixed prop="profile" label="配置" min-width="50">
              <template slot-scope="scope">
                  <span>{{scope.row.profile}}</span>
              </template>
            </el-table-column>

            <el-table-column fixed prop="label" label="标签" min-width="50">
              <template slot-scope="scope">
                  <span>{{scope.row.label}}</span>
              </template>
            </el-table-column>

            <el-table-column fixed prop="type" label="分类" min-width="50">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getListDeploy (param){
      let that = this
      this.$post('/epf-admin/getServerCfg',param).then(res=>{
        if (res.code != "0") {
          this.$message.error(res.msg); //失败
          return;
        }
