    ...
    <epf-container title="转让合同鉴证列表">
            ...
            <el-row :gutter="2">
				...
				<el-col :span="9">
					<div style="width:155px;float:left;margin-right:9px;">
						<el-select v-model="filterData.cantonProvinceName" size="small" placeholder="全部" @change="provinceSelect(dictionary.administrative_regions,filterData.cantonProvinceName,$event)">
							<el-option v-for="(item,index) in dictionary.administrative_regions" :ref="item.regionName" :id="item.id" :regionCode="item.regionCode" :label="item.regionName" :value="item.regionName" :key="index"></el-option>
						</el-select>
					</div>
					<div style="width:155px;float:left;margin-right:9px;">
						<el-select v-model="filterData.cantonCityName" size="small" placeholder="全部" @change="citySelect(dictionary.cantonCity,filterData.cantonCityName,$event)">
							<el-option v-for="(item,index) in dictionary.cantonCity" :ref="item.regionName" :id="item.id" :regionCode="item.regionCode" :label="item.regionName" :value="item.regionName" :key="index"></el-option>
						</el-select>
					</div>
					<div style="width:155px;float:left;">
						<el-select v-model="filterData.cantonAreaName" size="small" placeholder="全部" @change="areaSelect(dictionary.cantonAreaName,filterData.cantonAreaName,$event)">
							<el-option v-for="(item,index) in dictionary.cantonArea" :ref="item.regionName" :id="item.id" :regionCode="item.regionCode" :label="item.regionName" :value="item.regionName" :key="index"></el-option>
						</el-select>
					</div>
				</el-col>
				<el-col :span="1.5">
				...
				<el-col :span="3">
					<el-select size="small" class="filterCirculatemode" v-model="filterData.statusName" placeholder="全部" @change="statusSelect(dictionary.purchasr_status,filterData.statusName,$event)">
						<el-option v-for="(item,index) in status" :ref="item.zhCn" :id="item.id" :dictKey="item.dictKey" :label="item.zhCn" :value="item.zhCn" :key="index"></el-option>
					</el-select>
				</el-col>
			</el-row>
			<el-row :gutter="2" class="title_top">
				<el-col :span="2">
					<span class="filter_title">申请时间 :</span>
				</el-col>
				<el-col :span="6">
					<el-date-picker v-model="useDate" type="daterange" unlink-panels value-format="yyyy-MM-dd" range-separator="至" start-placeholder="起始时间" end-placeholder="截止时间" size="small" @change="selectDatarange"></el-date-picker>
				</el-col>
        ...
        <div class="epf_fr_box" :style="{height:boxHeight- 165 +'px'}">
            ...
            <div class="epf_fr_content contents epf_no_border" :style="{height:boxHeight - 205 +'px'}">
                <el-table 
                    :data="contartData" border v-loading="loading" 
                    ...
                    <el-table-column prop="contractNo" label="合同编号" width="240" align="left">
                        <template slot-scope="scope">
                            <span style="text-align:center;cursor:pointer;color: #4772d5;">{{ scope.row.contractNo }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="recordNo" label="合同统一编码" width="250" align="left"></el-table-column>
                    ...
                    <el-table-column prop="information" label="关联信息" width="120" align="left">
                        <template slot-scope="scope">
                            <span class="titleHover" @click="rowClickHandleDeal(scope.row)" style="text-align:center;cursor:pointer;" v-if="scope.row.relevanceCode=='2'">关联交易信息</span>
                            <span class="titleHover" @click="rowClickHandleSupply(scope.row)" style="text-align:center;cursor:pointer;" v-if="scope.row.relevanceCode=='1'">关联供给信息</span>
                            <span class="titleHover" style="text-align:center;" v-if="scope.row.relevanceCode=='3'">未关联</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="recordType" label="合同状态" width="200" align="left">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="操作" align="left" fixed="right" width="300">
                        <template slot-scope="scope">
                            <el-button size="mini" type="success" plain @click="viewDetail(scope.row.contractId,scope.row.procInsId,'transfer')">查看详情</el-button>
                    ...
                    <i class="el-icon-circle-close-outline" @click="delShow=false"></i>
                                    ...
                                    <div class="button_default" @click="delShow=false">取消</div>
                                </div>
                            </div>
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods: {
		...
		statusSelect(dic, status, event) {
			this.filterData.status = this.$refs[status][0].$attrs.dictKey;
		},
		selectDatarange() {
			//选择日期范围后的回调
