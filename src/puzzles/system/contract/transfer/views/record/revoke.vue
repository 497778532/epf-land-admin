					...
					<div class="publish_main_cont" style="margin-top:30px;">
                        ...
                        show-word-limit :disabled="disabled" style="width:60%;height:120px;"></el-input>
							...
							<el-form-item label="合同撤销附件">
								<div style="width:80%">
									<EPF-annexList :tableType="tableType" :tableNmae="tableNmae" :belong="belong" :areaCode="areaCode" :fileSize="6" :fileType="fileType" :annexType="annexType" v-if="belong!==''&&areaCode!==''"></EPF-annexList>
								</div>
							</el-form-item>
							<el-form-item>
	...
	export default {
		data() {
			return {
				...
				fileType: ["rar", "zip", "doc", "docx", "pdf", "jpg", "png", "xlsx"],
				annexType: "CONTRACT",
				tableType: false,
				title: "合同撤销审核详情",
				actInfo: {},
...
//				this.$bus.$emit("refreshPage", ["home"]);
			...
			examine(obj, buttonView) {
				...
				if(buttonView == "审核") {
					...
					this.$nextTick(() => {
						this.$bus.$emit("transferaudit", "view"); //查看审核结果
					});
				}
			},
			actResult(re, node) {
