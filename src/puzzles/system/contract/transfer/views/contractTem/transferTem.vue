			...
			<div class="signatureDetails_cont signlist0 signlist1">
				...
				<div class="signNum">
					<div class="signNumCon">
						<p v-if="transferTempData.contractStatus!=='contract_status-008'">合同编号：{{transferTempData.code}}</p>
						<p v-if="transferTempData.contractStatus=='contract_status-008'">合同统一编码：{{transferTempData.recordNo}}</p>
					</div>
				</div>
				...
				<div class="signlist signlist3">
					...
					<p>建设用地使用权转让期限为 {{transferTempData.term}} 年,即{{transferTempData.beginDate}}至{{transferTempData.endDate}}(转让期限不得超过原出让(或出租)合同约定的使用年限减去甲方已使用年限后的剩余年限)。</p>
				</div>
...
<script>
	...
	export default {
        ...
        this.dialogForm = true;
				...
				console.log("====obj==", obj)
				this.$nextTick(() => {
					thi.$bus.$emit("transferaudit", "examine"); //需要审核
				});
			},
			actResult(re, node) {
			...
			setState(state) {
				...
				let token = sessionStorage.getItem("token");
				console.log(
					"------------------------" + this.transferTempData.contractType
				);
				//改变状态
				Axios.post(
