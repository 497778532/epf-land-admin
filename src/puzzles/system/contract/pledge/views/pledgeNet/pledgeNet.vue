...
export default {
    ...
    methods: {
        ...
        auditResult(id, businessId, procDefId, procInsId, taskId, ifAudit) {
            ...
            let routerPath = `/system/contract/transfer/views/record/revoke?contractId=${id}&contractType=mortgage&businessId=${businessId}&procDefId=${procDefId}&procInsId=${procInsId}&taskId=${taskId}&auditStateTitle=${title}&ifAudit=${ifAudit}`;
            let item = {url:routerPath,menuname:title}
            //将当前点击的选项添加到vuex中
            this.$router.push({path:routerPath,query: {
					contractId: id,
        ...
        contractFiling(id, cantonArea) {
            let title = "合同鉴证";
            let routerPath = `/system/contract/transfer/views/record/recordDetail?contractId=${id}&&contractType=mortgage&&record=record&&cantonArea=${cantonArea}`;
            let item = {url:routerPath,menuname:title}
            //将当前点击的选项添加到vuex中
            this.$router.push({path:routerPath,query: {
					contractId: id,
