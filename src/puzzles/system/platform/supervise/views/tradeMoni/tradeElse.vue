<template>
  ...
  <div>
    <div class="epf_term_box">
        <el-row style="margin-bottom:10px;">
          ...
          <el-col :span="8">
            <div style="width:30%;float:left;margin-right:9px;">
              <el-select
                v-model="filterData.provincialCodeName"
                size="small"
                placeholder="全部"
                @change="provinceSelect(dictionary.administrative_regions,filterData.provincialCodeName,$event)"
            ...
            <div style="width:30%;float:left;margin-right:9px;">
              <el-select
                v-model="filterData.cityCodeName"
                size="small"
                placeholder="全部"
                @change="citySelect(dictionary.cityCode,filterData.cityCodeName,$event)"
                :disabled="cityCheck"
              >
                <el-option
                  v-for="(item,index) in dictionary.cityCode"
                  :ref="item.regionName"
                  :id="item.id"
                  :regionCode="item.regionCode"
                  :label="item.regionName"
            ...
            <div style="width:30%;float:left;">
              <el-select
                v-model="filterData.cantonCodeName"
                size="small"
                placeholder="全部"
                @change="areaSelect(dictionary.cantonCode,filterData.cantonCodeName,$event)"
                :disabled="AreaCheck"
              >
                <el-option
                  v-for="(item,index) in dictionary.cantonCode"
                  :ref="item.regionName"
                  :id="item.id"
                  :regionCode="item.regionCode"
                  :label="item.regionName"
          ...
          <el-col :span="3">
            <el-select
              size="small"
              v-model="filterData.natureName"
              placeholder="全部"
              @change="natureSelect(dictionary.land_nature,filterData.natureName,$event)"
            >
              <el-option
          ...
          <el-col :span="7">
            <div style="width:45%;float:left;margin-right:9px;">
              <el-select
                v-model="filterData.landUse1Name"
                placeholder="全部"
                @change="landUse1Select(dictionary,filterData.landUse1Name,$event)"
              >
                <el-option
            ...
            <div style="width:45%;float:left;">
              <el-select
                v-model="filterData.landUse2Name"
                placeholder="全部"
                @change="landUse2Select(dictionary,filterData.landUse2Name,$event)"
              >
                <el-option
        ...
        <el-row style="margin-bottom:10px;">
          <el-col :span="3">
            <span class="filter_title">鉴证申请时间：</span>
          </el-col>
          <el-col :span="7">
            <el-date-picker
              v-model="submitTime"
          ...
          <el-col :span="3">
            <el-select
              ...
              placeholder="全部"
              @change="landTypeSelect(dictionary.flow_mode,filterData.landTypeName,$event)"
            >
              <el-option
          ...
          <el-col :span="2">
            <span class="filter_title">关联供需：</span>
          </el-col>
          <el-col :span="3">
            <el-select
              size="small"
              v-model="filterData.isRelation"
              placeholder="全部"
            >
        ...
        <el-row style="margin-bottom:10px;">
          <el-col :span="3">
            <span class="filter_title">不动产权证号：</span>
          </el-col>
          <el-col :span="6">
            <el-input v-model="filterData.warrantno" size="small" placeholder="请输入地块编号"></el-input>
          </el-col>
    ...
    <div class="epf_fr_box" :style="{height:boxHeight- 195 +'px'}"> 
      ...
      <div class="epf_fr_content contents epf_no_border" :style="{height:boxHeight - 235 +'px'}">
        <el-table
          ...
          <el-table-column prop="warrantno" label="不动产权证号" width="220"></el-table-column>
          ...
          <el-table-column label="信息发布编号" width="220">
            <template slot-scope="scope">
              <p v-if="scope.row.publishNo">信息发布编号：{{scope.row.publishNo}}</p>
              <p>土地性质：{{scope.row.natureName}}</p>
              <p>用途：{{scope.row.landUse1Name}}/{{scope.row.landUse2Name}}</p>
            </template>
          </el-table-column>

          <el-table-column prop="contractNo" label="合同编号" width="230"></el-table-column>
          <el-table-column prop="createtime" label="鉴证申请时间" width="160"></el-table-column>
          <el-table-column prop="landTypeName" label="交易类型" width="100"></el-table-column>
          <el-table-column prop="recordTypeName" label="网签状态" width="100"></el-table-column>

          <el-table-column prop="transactionPrice" label="成交价格（万元）" width="160"></el-table-column>
          <el-table-column prop="secondparty" label="受让方/抵押权人/租赁方" width="230"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClickDetail(item) {
      let title = "自行交易详情";
      let routerPath = `/system/platform/supervise/views/tradeMoni/detailElse?contractId=${item.contractId}`;
      let itemObj = {url:routerPath,menuname:title}
      //将当前点击的选项添加到vuex中
      this.$router.push({path:routerPath,query: { contractId: item.contractId } } );
    ...
    provinceSelect(dic, cantonCode, event) {
      ...
      this.filterData.provincialCode = this.$refs[cantonCode][0].$attrs.regionCode;
    ...
    landUse1Select(dic, landUse1, event) {
      ...
      this.filterData.landUse1 = this.$refs[
        landUse1
      ][0].$attrs.dictKey;
      this.$get(
    ...
    landUse2Select(dic, landUse1, event) {
      this.filterData.landUse2 = this.$refs[
        landUse1
      ][0].$attrs.dictKey;
    },
    landTypeSelect(dic, landTypeName, event) {
      this.filterData.landType = this.$refs[
        landTypeName
      ][0].$attrs.dictKey;
    },
    ...
    resetFilter() {
      ...
      for (let item in this.filterData) {
        if((this.provinceCheck && item == 'provincialCode') || (this.provinceCheck && item == 'provincialCodeName')){
          continue;
        }else if((this.cityCheck && item == 'cityCode') || (this.cityCheck && item == 'cityCodeName')){
          continue;
        }else if((this.AreaCheck && item == 'cantonCode') || (this.AreaCheck && item == 'cantonCodeName')){
          continue;
        }else{
  ...
  mounted(){
    setTimeout(()=>{
      if(sessionStorage.getItem("org_level") == 'org_level-003'){
        this.filterData.provincialCode = sessionStorage.getItem("jgProvinceCode")
        this.filterData.provincialCodeName = sessionStorage.getItem("jgProvince")
