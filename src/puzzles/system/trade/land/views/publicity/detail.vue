              ...
              <div class="landshow_main_cont">
                ...
                <table class="landshow_table1" width="100%" cellpadding="0" cellspacing="0">
                  ...
                  <tr v-for="(item,index) in transGoodsEntityList" :key="index">
                    <td>{{item.goodsNo}}</td>
                    <td>{{item.cantonProvinceName}}-{{item.cantonCityName}}-
                      {{item.cantonAreaName}}{{item.location?'-'+item.location+'，':''}}
                      {{item.east?'东至'+item.east+'，':''}}
                      {{item.west?'西至'+item.west+'，':''}}
                      {{item.south?'南至'+item.south+'，':''}}
                      {{item.north?'北至'+item.north:''}}
                    </td>
                    <td>{{item.area}}</td>
                    <td>{{item.years}}</td>
...
export default {
  ...
  methods:{
    queryData(){
      this.$get(this.$API.EPF_TRADE.PUBLICITY.PUBLICITY_DETAIL,this.params).then(res=>{
