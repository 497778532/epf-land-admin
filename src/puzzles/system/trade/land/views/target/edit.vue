      ...
      <div>
        <el-dialog title="选择地块信息" :visible.sync="centerDialogVisible" width="60%">
          ...
          >
            ...
            <el-table-column prop="insertUser" label="经办人"  width="150" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    saveTransTarget: function() {
      ...
      this.$useAppend(this.$API.EPF_TRADE.TARGET.TARGET_EDIT, {
    ...
    async getPraceData() {
      ...
      let res = await this.$get(this.$API.EPF_TRADE.TARGET.TARGET_GOODS_LIST+`?page=${
          this.pageNo
        }&rows=${this.pageSize}&isBinding=1&goodsNo=${
          this.filterData.landNo
        }&firstClassUse=${this.filterData.landUse}`,this.filterData
      );
      console.log(res);
      if (res.code == "0") {
    ...
    getItemData() {
      ...
      this.$get(this.$API.EPF_TRADE.TARGET.TARGET_INFO+`?targetId=`+id
      ).then(res => {
        // this.mydata = res.map;
        ...
        this.beginNoticeTime=res.map.transTargetObj.beginNoticeTime;
        this.beginApplyTime=res.map.transTargetObj.beginApplyTime;
        this.beginFocusTime=res.map.transTargetObj.beginFocusTime;
        this.endFocusTime=res.map.transTargetObj.endFocusTime;
        this.endEarnestTime=res.map.transTargetObj.endEarnestTime;
        this.beginLimitTime=res.map.transTargetObj.beginLimitTime;
        let transTypeMark = "";
