                    ...
                    <li>
                      <el-form-item label="交易中心：" prop="tradingCenter">
                        <!-- <el-select
                          ...
                          style="width:360px;"
                        >
                          <el-option label="济南市二级市场交易中心" value="74a1e654-5065-41e9-b6b8-aa273390fd27"></el-option>
                          <el-option label="太原市二级市场交易中心" value="42ec49e2-e430-4213-ba14-6ad14af1e5d8"></el-option>
                        </el-select>-->
...
export default {
  ...
  async beforeRouteEnter(to, from, next) {
    ...
    try {
      ...
      if (!targetId) {
        ...
        next(vm => {
          vm.mydata.transTarget.id = vm.getRandow();
        });
      } else {
        console.log(22222222222222222222222);
        const data = await this.$get(this.$API.EPF_TRADE.TARGET.TARGET_INFO+`?targetId=${targetId}`
        );
        next(vm => {
          vm.mydata = data.map;
        });
  ...
  methods: {
    ...
    saveTransTarget: function() {
      ...
      this.$useAppend(this.$API.EPF_TRADE.TARGET.TARGET_ADD, {
        ...
        this.$bus.$emit("landMarkSave", true);
    ...
    async getPraceData() {
      ...
      let res = await this.$get(this.$API.EPF_TRADE.TARGET.TARGET_GOODS_LIST,
        this.filterData
      );
      console.log(res);
      if (res.code == "0") {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          // 成功 提交数据并下一步
          // let res = await this.$post("/epf-trade/admin/transtarget/addTransTarget", this.mydata.transTarget);
