    ...
    <epf-container title="转入托管账户">
            ...
            <el-form ref="filterData" :model="filterData">
                <el-row class="mainAccounts">
                    ...
                    <el-col :span="5" style="text-align:right">
                        <button class="epf_btn epf_btn_bg" @click="getBidShiftToTrusteeshipAccount({realName:filterData.Bidder,targetId:targetId})"> 查 询</button>
        ...
        <div class="epf_fr_box" :style="{height:boxHeight- 165 +'px'}">
            <div class="epf_toolbar">
                <span class="toolsTitle">竞买保证金转入托管账户</span>
                <span>标的编号:</span>
                <em>{{targetNo}}</em>
                <span>流转方式:</span>
                <em>{{businessType}}</em>
            ...
            <div class="epf_fr_content contents epf_no_border" :style="{height:boxHeight - 205 +'px'}">
                <el-table
                    ...
                    <el-table-column prop="isValid" label="是否使用" width="120" align="left"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        getBidShiftToTrusteeshipAccount: async function(params) {
            const res = await this.$get(this.$API.EPF_TRADE.FINANCE.DEPOSIT_LIST, params);
            if (res.code == "0") {
        ...
        overTo: async function(myid) {
            const res = await this.$get(
                this.$API.EPF_TRADE.FINANCE.DEPOSIT_ACCOUNT,
                {
                    targetId: this.$route.query.targetId,
                    ids: myid
                }
