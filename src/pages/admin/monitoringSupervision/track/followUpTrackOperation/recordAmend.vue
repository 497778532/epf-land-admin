...
export default {
  ...
  methods: {
    ...
    async getFollowMsg() {
      ...
      let res = await this.$post(
        `/epf-monitor/monitor/goodTrack/queryGoodTrackRecordById`,
        { id: id }
      );
      console.log(res);
      if (res.code == "0") {
        ...
        this.imgAxios(
          this.followData.prospectImg,
          this.prospectFileList,
          "prospectpId"
        );
    ...
    saveTrack() {
      ...
      this.$post(
        "/epf-monitor/monitor/goodTrack/updateGoodTrackRecordById",
        params
      ).then(res => {
        console.log(res);
        if (res.code != "0") {
        ...
        // Bus.$emit("followUpTrackEditorSave", true);
  ...
  created() {
    ...
    this.getFollowMsg(this.$route.query.id);
  },
  computed: {
