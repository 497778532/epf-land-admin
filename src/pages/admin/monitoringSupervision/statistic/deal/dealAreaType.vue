    ...
    <div class="filter_wrap">
      ...
      >
        ...
        >
          ...
          <div
            id="cycleChart"
            style="width:100%;height:550px;margin-bottom:75px;"
          ></div>
        </el-tab-pane>
        <el-tab-pane label="列表" name="list" style="height:100%;">
          <div
            class="toolsTitleBox_graph"
            style="background-color:#fff;border:0;"
          >
          ...
          >
            ...
            <el-table-column label="土地性质" min-width="150">
              <template slot-scope="scope">
                ...
                <span v-if="scope.row.landChar != 'all'">{{
            ...
            <el-table-column label="交易状态" min-width="120">
              ...
              <span v-if="filterData.trans_statusName != ''">{{
                filterData.trans_statusName
              }}</span>
              <!-- </template> -->
            </el-table-column>
            ...
            <el-table-column label="时间" min-width="120">
              <template slot-scope="scope">
                <span v-if="timeCycleName == 'MOUTH'">{{
                  scope.row.yearMonth
                }}</span>
                <span v-if="timeCycleName == 'SEASON'"
                  >{{ scope.row.year }}年第{{ scope.row.season }}季度</span
                >
                <span v-if="timeCycleName == 'YEAR'"
...
export default {
  ...
  methods: {
    ...
    getList(val) {
      ...
      // this.$get(`/epf-monitor/statistics/trans/queryGroupByTimeOfCirculationList2`,params).then(res => {
      this.$get(
        `/epf-monitor/statistics/trans/queryGroupByTimeOfCirculationList`,
        params
      ).then(res => {
        console.log(res);
        if (res.code == "0") {
