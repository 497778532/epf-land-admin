<template>
  <div class="page-body">
      ...
      <div class="switchHeight_box">
        ...
        <el-row style="margin-top:10px;margin-bottom:10px;">
          <el-col :span="6" :offset="1">
            <div class="btnBox">
              <div class="button_bg" @click="add" style="margin:0;">发邮件</div>
    ...
    <!-- 新增弹窗 -->
    <el-dialog title="发邮件" class="companyInfo" width="900px" :visible.sync="addPopup" :append-to-body='true' :close-on-click-modal="false">
        <div class="middleBox">
        <el-form :model="msgMessage" :rules="rules" ref="msgMessage" label-width="90px" label-position="left" size="small">
        
        <el-form-item label="收件人邮箱" prop="emails">
            <el-input v-model="msgMessage.emails" clearable placeholder="多邮箱使用‘;’（英文）分开"></el-input>
        </el-form-item>
        <el-form-item label="邮件标题" prop="title">
            <el-input v-model="msgMessage.title" clearable></el-input>
        </el-form-item>
        <el-form-item label="邮件类容" prop="content">
            <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 6}" v-model="msgMessage.content" clearable></el-input>
        </el-form-item>
        ...
        <div class="middleBox_btn" style="text-align:center;">
            <div class="button_default" @click="addPopup = false">保存</div>
            <div class="button_bg" @click="addData('msgMessage')">发送</div>
        </div>
...
import selectInput from '@/components/common/form/selectInpt'
  export default {
    ...
    methods: {
      ...
      addData(msgMessage){
        var that = this;
        if(msgMessage.title==""||msgMessage.content==""){
            return
        }
        //调用添加接口传输数据
        that.$post('/epf-admin/admin/msgmessage/addMsgMessage',that.msgMessage).then(res => {
            if(res.code != '0'){
