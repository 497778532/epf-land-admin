              ...
              <el-col :span="10">
                <el-form-item label="行政区" prop="cantonAreaName">
                  <el-select
                    ...
                    placeholder="请选择省"
                    @change="provinceSelect(dictionary.administrative_regions,sizeForm.cantonProvinceName,$event)"
                  >
                    <el-option
              ...
              <el-col :span="7">
                <el-form-item prop="cantonCity" class="canton_Regions" style="margin-left:0;">
                  <el-select
                    ...
                    placeholder="请选择市"
                    @change="citySelect(dictionary.cantonCity,sizeForm.cantonCityName,$event)"
                  >
                    <el-option
              ...
              <el-col :span="7">
                <el-form-item prop="cantonArea" class="canton_Regions">
                  <el-select
                    ...
                    placeholder="请选择区/县"
                    @change="areaSelect(dictionary.cantonAreaName,sizeForm.cantonAreaName,$event)"
                  >
                    <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        contact: [
          { min: 2, max: 10, message: "请输入正确的权属人." },
        ...
        address: [
          { min: 5, max: 40, message: "长度在 5 到 40个字符" },
          {
            pattern: /^[\u4E00-\u9FA5][\u4E00-\u9FA5A-Za-z0-9]+$/,
            message: "必须以中文开头,只能输入中文、英文、数字."
          }
        ],
        bankSimple: [
  ...
  methods: {
    ...
    getTransBankList(params) {
      ...
      this.$get("/epf-trade/admin/transbank/getTransBankList", params).then(res => {
        if (res.code == "0") {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.currentHandle == "editor") {
            //  编辑操作
            this.$post("/epf-trade/admin/transbank/editTransBank",this.sizeForm).then(res => {
              if (res.code ==0) {
                this.$message({
                  message: "修改成功",
                  type: "success"
            ...
            this.$post("/epf-trade/admin/transbank/addTransBank",this.sizeForm).then(res => {
              if (res.code ==0) {
                this.$message({
                  message: "成功新增一条数据!",
                  type: "success"
    ...
    del(row){
      ...
      this.$confirm("确认删除该条的信息，是否继续?", "提示", {
        ...
        .then(async () => {
          this.$post("/epf-trade/admin/transbank/deleteTransBank", {
              id: row.id
            }).then(res => {
              if (res.code ==0) {
                this.$message({
    ...
    accountManageOpen(row){
      let routerPath = `/accountPage?bankId=${row.id}&bankName=${row.name}&bankNo=${row.no}`;
      let title = "账号管理";
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
        path: "/accountPage",
        query:{bankId:row.id,bankName:row.name,bankNo:row.no}
      });
    },
