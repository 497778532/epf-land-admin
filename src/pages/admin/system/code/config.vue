<template>
  <div class="app-container" :style="{height:clientHeight}" style="overflow-y:auto;">
    <el-row :gutter="15">
...
export default {
  ...
  created() {
    ...
    that.tableHeight = document.documentElement.clientHeight - 385
    that.tableName = that.$route.query.tableName
    
      that.init()

      that.getform(that.tableName)
  ...
  methods: {
    async init() {
      ...
      that.loading = true
      that.$get(that.url, that.params).then(res => {
    ...
    getform(tableName){
      var that=this;
      that.$get('/epf-generator/api/genConfig/'+tableName).then(res => {
        that.form = res
        that.form.cover = that.form.cover.toString()
      });
    },
    beforeInit() {
      this.url = '/epf-generator/api/generator/columns'
    ...
    saveColumnConfig() {
      ...
      that.$put('/epf-generator/api/generator/save',that.data).then(res => {
    ...
    doSubmit() {
      ...
      that.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          that.$put('/epf-generator/api/genConfig',that.form).then(res => {
