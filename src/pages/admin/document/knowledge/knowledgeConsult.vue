        ...
        <el-main style="margin-right:15px;" >
          <el-tabs>
            <div class="switchHeight_box">
              <el-row :gutter="2">
                  <el-col :span="19" style="padding-left: 2%;padding-top: 5px;">
                    <span style="font-size: 28px;font-weight: bold;">上传记录</span>
                  </el-col>
                  ...
                  <!-- 搜索切换 -->
                  <el-col :span="4" class="searchIpt" v-show="!searchShow" style="padding-right: 10px;">
            ...
            <div style="background:#fff">
              <el-table
                ...
                v-show="tablelist"
                >
                <el-table-column prop="examineState" label="审核状态" min-width="70">
                  <template slot-scope="scope">
                    <span style="color:#2B6EE2" v-if="scope.row.examineState == '0'">待审核</span>
                    <span style="color:#E02020" v-else-if="scope.row.examineState == '1'">审核不通过</span>
                    <span style="color:#389E0D" v-else-if="scope.row.examineState == '2'">审核通过</span>
                  </template>
                </el-table-column>
                <el-table-column prop="name" label="项目名称"  min-width="230" >
                  <template slot-scope="scope">
                    <div class="cell_click" @click="clickselectrow(scope.$index, scope.row)">
                      <div class="docuImg" v-if="scope.row.extName!=null">
                ...
                <el-table-column prop="examineTime"  label="审核时间" min-width="130">
                  <template slot-scope="scope">
                    {{scope.row.examineTime}}
                  </template>
                </el-table-column>
                <el-table-column prop="examineComments" label="审核意见" min-width="150">
                  <template slot-scope="scope">
                ...
                <el-table-column prop="docNo" label="操作" min-width="70">
                  <template slot-scope="scope">
                    <div class="cell_click" @click="uploadWrokFile(scope.$index, scope.row)">
                      <span style="color:#4E88FE;" v-if="scope.row.examineState == '1'">重新上传</span>
                    </div>
                  </template>
                </el-table-column>
              </el-table>
...
export default {
  ...
  methods: {
    ...
    getDocList(params) {
      ...
      that.loading = true;
      // 公共文档上传记录
      if(that.folderType == '1'){
        that.$get("/epf-document/docdocumentinfo/getMyUploadPlanningDocs", params).then(res => {
          console.log("公共文档上传记录===========", res);
          if (res.code != "0") {
