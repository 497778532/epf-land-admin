        ...
        <el-main style="margin-right:15px;" >
          <el-tabs>
            <div class="switchHeight_box">
              <el-row :gutter="2">
                  <el-col :span="11" style="padding-left: 2%;padding-top: 5px;">
                    <span style="font-size: 28px;font-weight: bold;">公共文档</span>
                    <p class="tabs">
...
export default {
  ...
  methods: {
    queryDictList() {
      let that = this;
      let codes=["involved_industry","involved_bussyniss","doc_sourse"];
      that.$post('/epf-admin/admin/dict/getDictDataByCode', {'codes': codes}).then(res => {
        ...
        let docSources=data.doc_sourse;
        let list=[];
        for (let index = 0; index < docSources.length; index++) {
          list.push({name:docSources[index]["zhCn"],value:docSources[index]["zhCn"]})
    ...
    querySearch(queryString, cb) {
     ...
     clear(){
      ...
      saveuserinfo() {
        ...
        that.$post("/epf-document/admin/doctag/updateUtag", {id:ids,type:"0"}).then(res => {
    ...
    getDocList(params) {
      ...
      that.loading = true;
      params["foldertype"]='1';
      that.$get("/epf-document/docdocumentinfo/getTenantDocJson", params).then(res => {
          console.log("871===========", res);
          if (res.code != "0") {
            that.$message.error(res.msg); //失败
            that.loading = false;
        ...
        that.cities=[];
          for(var i=0 ; i< this.docInfoList.length ; i++){
            that.cities.push(this.docInfoList[i].id);
          }
        that.loading = false;
        });
      },
      ...
      uploadWrokFile(){
        ...
        let tabObj = {title, routerPath}
        that.setTabsList(tabObj)
        that.$router.push({path: routerPath,query:{folderType : that.folderType,uplaodtype:"0"}})
      },
      //上传记录
      uploadRecord(){
        let that = this;
      ...
      setName(node, data) {
       ...
       //知识管理夹
        that.$post("/epf-document/admin/docfolder/save", {folderName: name,pId: this.parentdata.id,folderType:"1"})
          .then(res => {
            if (res.code != 0) {
