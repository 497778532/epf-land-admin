    ...
    <div class="filter_wrap">
      ...
      <div class="switchHeight_box" style="padding-top:0">
        <div class="toolsTitleBox">
          ...
          <div class="button_export" @click="xlsExport">导出</div>
        </div>
        <el-table
          :data="tableData1"
          v-loading="loading"
...
export default {
  ...
  methods: {
    rowClickHandle(row, event, column) {
      ...
      this.$bus.$emit("closeTabsItem", "demandInfoDetails");
      let title = row.title;
      let routerPath =
        "/demandInfoDetails?supply=" +
        row.id +
        "&actInfo=" +
        qs.stringify({
    ...
    xlsExport() {
      ...
      btn.setAttribute(
        "href",
        "/api/epf-supply/tpurchase/wantBuyXls?demand=need_rent"
      ); // href链接
      btn.click(); // 自执行点击事件
    },
