    ...
    <el-tabs v-model="activeTabs" type="border-card" @tab-click="handleClick">
        ...
        <div class="switchHeight_box" style="padding-top:0">
          ...
          <el-table
           ...
           >
            ...
            <el-table-column prop="contractName" label="合同名称" align="left"></el-table-column>
      ...
      <el-tab-pane label="已处理" name="processed">
        <div class="switchHeight_box" style="padding-top:0">
          ...
          <el-table
           ...
           >
            ...
            <el-table-column label="操作" width="400" fixed="right" align="left">
              <template slot-scope="scope">
                <el-button size="mini" type="success" plain  @click="viewDetail(scope.row.id,scope.row.procInsId,'mortgage')">查看详情</el-button>
          ...
          <el-dialog title="查看审核结果" width="40%" :visible.sync="dialogForm" :append-to-body="true">
            <template>
              <div id="dialog">
...
export default {
  ...
  methods: {
    ...
    handleClickDetail(contractId){
      this.$bus.$emit("closeTabsItem", "mortgageContractTemplate");
      let routerPath = `/mortgageContractTemplate?contractId=${contractId}&contractView=1`;
      let title = "合同预览";
    ...
    rowClickHandle(row, event, column) {
      ...
      // this.setTabsList(tabObj)
      // this.$router.push({path:'/demandSupplyDetails',query:{supply:row.relevanceId}})
    },
    supply(row) {
    ...
    examine(id, businessId, procDefId, procInsId, taskId) {
      this.$bus.$emit("closeTabsItem", "mortgageContractTemplate");
      let routerPath = `mortgageContractTemplate?contractId=${id}&businessId=${businessId}&procDefId=${procDefId}&procInsId=${procInsId}&taskId=${taskId}`;
      let title = "抵押合同审核详情";
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
    ...
    revotrail(id) {
      ...
      let title = "合同撤销申请";
      let ifAudit = "";
      let routerPath = `/ContractRevotrailAudit?contractId=${id}&contractType=mortgage&auditStateTitle=${title}&ifAudit=${ifAudit}`;
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
    ...
    auditResult(id, businessId, procDefId, procInsId, taskId, ifAudit) {
      ...
      let routerPath = `/ContractRevotrailAudit?contractId=${id}&contractType=mortgage&businessId=${businessId}&procDefId=${procDefId}&procInsId=${procInsId}&taskId=${taskId}&auditStateTitle=${title}&ifAudit=${ifAudit}`;
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
        path: "/ContractRevotrailAudit",
    ...
    // 不动产自动核验
    autoExamine() {
      this.$bus.$emit("closeTabsItem", "autoInspection");
    ...
    viewDetail(id, procInsId, contract) {
      this.$bus.$emit("closeTabsItem", "filingDetails3");
      let routerPath = `/filingDetails3?contractId=${id}&procInsId=${procInsId}&contractType=${contract}`;
      let title = "抵押合同归档详情";
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
    ...
    contractFiling(id, cantonArea) {
      ...
      let routerPath = `/contractFiling?contractId=${id}&&contractType=mortgage&&record=record&&cantonArea=${cantonArea}`;
      let title = "合同鉴证";
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
    ...
    exportr() {
      ...
      window.location.href = `/api/epf-contract/transContract/downContract?token=${token}&isCheck=${this.isCheck}&contractType=mortgage&status=${this.filterData.status}&code=${this.filterData.code}&beginDate=${this.filterData.beginDate}&endDate=${this.filterData.endDate}&cantonCode1=${this.filterData.cantonCode1}&cantonCode2=${this.filterData.cantonCode2}&cantonCode3=${this.filterData.cantonCode3}`;
    }
  }
};
...
//   this.currentTabs = val.name;
//   this.tableData1 = [];
//   // if (val.activeTabs == "pending") {
//   //   this.init("2", this.pageNo, this.pageSize);
//   //   return Promise.resolve();
//   // } else if (val.activeTabs == "processed") {
//   //   this.init("1", this.no_pageNo, this.no_pageSize);
//   //   return Promise.resolve();
//   // }
