      ...
      <div class="signatureDetails_cont signlist0 signlist1">
        ...
        <div class="signNum">
          <div class="signNumCon">
            <p v-if="rules.contractStatus!=='contract_status-008' && rules.contractStatus!=='contract_status-005'">合同编号：{{rules.code?rules.code:''}}</p>
            <p v-if="rules.contractStatus=='contract_status-008' || rules.contractStatus =='contract_status-005'">合同鉴证号：{{rules.recordNo}}</p>
          </div>
        </div>
        <div class="signtitle">
          {{rules.contractName}}
          ...
          <div v-if="rules.contractStatus==='contract_status-003'" class="contractCancel">
           ...
           <div v-if="rules.contractStatus==='contract_status-008'" class="contractsuccess">
            <span></span>鉴证成功
          </div>
          <div v-if="rules.contractStatus==='contract_status-011'" class="contractnosuccess">
            <span></span>鉴证失效
          </div>
        </div>
        ...
        <div class="signlist signlist3">
          ...
          <p>4.主债权数额:（小写）{{rules.amountOfPrincipal/10000}}万元 （大写） {{NumToChinese(rules.amountOfPrincipal)}}</p>
