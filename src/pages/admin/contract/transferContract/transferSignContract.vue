        ...
        <div class="switchHeight_box" style="padding-top:0">
          ...
          <el-table
            ...
            >
              <template slot-scope="scope">
                <span
                  style="color: #4772d5;cursor: pointer;"
                  @click="handleClickDetail(scope.row.contractId)"
                  >{{ scope.row.code }}</span
                >
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <span v-if="scope.row.signType == 'transaction_mode-001'"
                  >委托交易</span
                >
                <span v-if="scope.row.signType == 'transaction_mode-002'"
                  >自行交易</span
                >
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  plain
                  @click="
                    viewDetail(scope.row.id, scope.row.procInsId, 'transfer')
                  "
                  >查看详情</el-button
                >
                <el-button
                  ...
                  plain
                  v-if="scope.row.contractStatus == 'contract_status-004'"
                  @click="findAudit(scope.row, scope.$index)"
                  >查看审核结果</el-button
                >
                <el-button
                  size="mini"
                ...
                <el-button
                  ...
                  plain
                  v-if="scope.row.contractStatus == 'contract_status-005'"
                  @click="contractFiling(scope.row.id, scope.row.cantonArea)"
                  >合同鉴证</el-button
                >
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClickDetail(contractId) {
      this.$bus.$emit("closeTabsItem", "transferContractTemplate");
    ...
    transactions(row) {
      let routerPath =
        "/toEntrustDetail?tgId=" + row.relevanceId + "&id=" + row.relevanceId;
      let title = "交易详情页";
      let tabObj = { title, routerPath };
      //将当前点击的选项添加到vuex中
    ...
    auditResult(id, businessId, procDefId, procInsId, taskId, ifAudit) {
      ...
      let routerPath = `/ContractRevotrailAudit?contractId=${id}&contractType=transfer&businessId=${businessId}&procDefId=${procDefId}&procInsId=${procInsId}&taskId=${taskId}&auditStateTitle=${title}&ifAudit=${ifAudit}`;
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
        path: "/ContractRevotrailAudit",
    ...
    contractFiling(id, cantonArea) {
      ...
      let routerPath = `/contractFiling?contractId=${id}&&contractType=transfer&&record=record&&cantonArea=${cantonArea}`;
      let title = "合同鉴证";
      let tabObj = { title, routerPath };
      this.setTabsList(tabObj);
      this.$router.push({
...
.delDialog_box .container_box .title_box {
  width: 100%;
  height: 45px;
  padding: 0 20px;
...
.delDialog_box .container_box .title_box h4 {
  font-size: 18px;
  float: left;
  color: #323232;
}

.delDialog_box .container_box .title_box span {
  font-size: 26px;
  float: right;
  color: #b3b3b3;
}

.delDialog_box .container_box .title_box span i {
  cursor: pointer;
}

.delDialog_box .container_box .container {
  width: 100%;
  margin-top: 15px;
  height: 70px;
  padding-top: 25px;
