  ...
  <div class="page-body">
        ...
        <div style="padding:30px;">
          <EPF-switchTitle titles_h='竞买申请步骤' :shutOff='false'>
            <el-row class="searchBox">
              ...
              <el-col :span="6">
                <div class="button_bg" style="margin-left:20px;" @click="pendingParamsSearch">查询</div>
              </el-col>
            </el-row>
            <div class="addRule">
              <span class="titleRule">竞买申请步骤</span>
              <el-button class="primary addInput" :disabled="!modify" @click="addTableData()">
                新增
              </el-button>
            </div>
            <div class="tableBox">
        ...
        <div style="padding:30px;">
          <EPF-switchTitle titles_h='交易方式' :shutOff='false'>
            <el-row class="searchBox">
              ...
              <el-col :span="6">
                <div class="button_bg" style="margin-left:20px;" @click="processedParamsSearch">查询</div>
              </el-col>
            </el-row>
            <div class="addRule">
              <span class="titleRule">交易方式</span>
              <div class="button_bg" @click="addTransaction('add','')" style="margin:0;float:right;">新增</div>
            </div>
            <el-table
              :data="processedData"
              border
        ...
        <div style="padding:30px;">
          <EPF-switchTitle titles_h='节点管理' :shutOff='false'>
            <el-row class="searchBox">
              ...
              <el-col :span="6">
                <div class="button_bg" style="margin-left:20px;" @click="nodeParamsSearch">查询</div>
              </el-col>
            </el-row>
            <div class="addRule">
              <span class="titleRule">节点管理</span>
              <div class="button_bg" @click="addTrasNode" style="margin:0;float:right;">新增</div>
            </div>
            <el-table
              :data="nodeData"
              border
    ...
    <EPF-switchTitle titles_h='业务模型' :shutOff='false' style="margin-top:25px;">
      <div class="addRule" style="margin-top:25px;">
        ...
        <div class="button_bg" style="margin:0;float:right;" @click="addBusModel('add','')">新增</div>
      </div>
      <el-table
        :data="paginatorData"
        border
  ...
  export default {
    ...
    methods: {
      ...
      addTransaction (type,row) { //新增&修改交易方式页面
        if (type != 'upd') {
          let routerPath = '/addTransactionRule'
          let title = '新增交易方式'
          let tabObj = {title, routerPath}
          this.setTabsList(tabObj)
          this.$router.push({path: '/addTransactionRule'})
        } else {

          //获取所点击的节点ID，并且跳转页面
          console.log('获取的交易方式ID是：' + row.id)
          let title = '修改交易方式'
          let routerPath = '/addTransactionRule?id=' + row.id
          let tabObj = {title, routerPath}
          this.setTabsList(tabObj)
          this.$router.push({path: '/addTransactionRule', query: {id: row.id}})
        }
      },
      addBusModel (type,row) { //新增&修改业务模型页面
        if (type != 'upd') {
          let routerPath = '/addBusModel'
          let title = '新增业务模型'
          let tabObj = {title, routerPath}
          this.setTabsList(tabObj)
          this.$router.push({path: '/addBusModel'})
        } else {
          //获取所点击的节点ID，并且跳转页面
          let title = '修改业务模型'
          let routerPath = '/addBusModel?id=' + row.id
          let tabObj = {title, routerPath}
          this.setTabsList(tabObj)
          this.$router.push({path: '/addBusModel', query: {id: row.id}})
        }
      ...
      processedParamsList (Params) {
        //调用接口查询交易方式接口数据
        this.$get('/epf-tradeengine/transtransactiontype/getTransTransactionTypeJson', Params).then(res => {
          // res 是返回结果
          if (res.code != 0) {
      ...
      pendingParamsList (Params) {
        //调用接口查询竞买申请步骤列表接口数据
        this.$get('/epf-tradeengine/admin/translicensestep/getTransLicenseStepJson', Params).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      paginatorParamsList (Params) {
        //业务模型查询节点列表接口数据
        this.$get('/epf-tradeengine/admin/transbusinessmodel/getTransBusinessModelJson', Params).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      pendingParamsAdd (Params) {
        //调用接口添加竞买申请步骤数据
        this.$post('/epf-tradeengine/admin/translicensestep/addTransLicenseStep', Params).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      pendingParamsById (id) {
        //调用接口添加竞买申请步骤数据
        this.$get('/epf-tradeengine/admin/translicensestep/getTransLicenseStep', {id: id}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      pendingEdit (row) {
        let self = this
        //调用接口修改竞买申请步骤数据
        this.$post('/epf-tradeengine/admin/translicensestep/editTransLicenseStep', row).then(res => {
          // res 是返回结果
          if (res.code != 0) {
      ...
      pendingDel (row) {
        let self = this
        //调用接口修改竞买申请步骤数据
        this.$post('/epf-tradeengine/admin/translicensestep/deleteTransLicenseStep', {id: row.id}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            self.$message.error(res.msg)
            return
      ...
      pendingCheck (row) {
        ...
        //通过ID判断该条数据是否被引用中
        //调用接口修改竞买申请步骤数据
        this.$get('/epf-tradeengine/admin/translicensestep/getDeleteTransLicenseStep', {id:row.id}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            self.$message.error(res.msg)
            return
      ...
      nodeCheck (row) {
        //判断当前节点是否可以被删除
        //checkTransNode
        this.$get('/epf-tradeengine/transnodetemplate/checkTransNode', {id: row.id}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      deleteTransNode () {
        //调用删除接口进行数据的删除
        this.$get('/epf-tradeengine/transnodetemplate/deleteTransNodeTemplate', {id: this.nodeID}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
      ...
      nodeUpd (row) {
        ...
        let title = '修改节点'
        let routerPath = '/addTransactionNode?id=' + row.id
        let tabObj = {title, routerPath}
        this.setTabsList(tabObj)
        this.$router.push({path: '/addTransactionNode', query: {id: row.id}})
      },
      TransactionCheck(row){
        //删除交易信息
        //调用接口判断关联表是否存在数据
        this.$get('/epf-tradeengine/transtransactiontype/checkTransType', {id: row.id}).then(res => {
          // res 是返回结果
          if (res.code != 0) {
            this.$message.error(res.msg)
            return
          ...
          if (res.isTransType) {
            ...
            this.deleteNodeNo = false
            //调用接口进行删除操作
            this.$get('/epf-tradeengine/transtransactiontype/deleteTransTransactionType', {id: row.id}).then(res2 => {
              // res 是返回结果
              if (res2.code != 0) {
      ...
      deleteRuleData(id){
        //根据业务模型删除数据的接口
        //调用接口进行删除操作
        this.$post('/epf-tradeengine/admin/transbusinessmodel/deleteTransBusinessModel', {id: id}).then(res2 => {
          // res 是返回结果
          if (res2.code != 0) {
            this.$message.error(res2.msg)
            return
