              ...
              <div class="EntrustTra_main" style="margin:0 auto;" v-if="Enclosure">
                ...
                <EPF-switchTitle titles_h="委托附件清单" :shutOff="false" background="#fff">
                  <div class="EntrustTra_main_cont">
                    ...
                    <span class="search_input submit_input fl" @click="submitLand('save')">保存</span>
                  </div>
                </EPF-switchTitle>
              </div>
            </div>
  ...
  data() {
    ...
    const buildingDensityVa = (rule, value, callback) => {
      if (
        ...
        if (
          this.transGoods.buildingDensity > this.transGoods.buildingDensity2
        ) {
          callback(new Error("最大值不能小于最小值!"));
        } else {
          callback();
    ...
    return {
      ...
      rules: {
        ...
        contactAddress: [
          // {
          //   required: true,
          //   message: "请输入联系人/代理人联系地址",
          //   trigger: "blur"
          // },
          { validator: (rule, value, callback) =>{this.validateNum(rule, value, callback,0,100,'长度在100字符以内（注意若是汉字，1个汉字是2个字符）')} ,trigger: 'blur' },
          {
            pattern: /^[\u4E00-\u9FA5A-Za-z0-9\、\,\，\_\-\———\【】()（）]+$/,
            message:
              "可以输入中文、英文、数字及部分标点符号，顿号、逗号、连接号（-）、破折号、括号（）【】"
