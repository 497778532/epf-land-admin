    ...
    <div class="filter_wrap">
      <div>
        ...
        <table class="titleData">
          ...
          <tr>
            <td style="width:140px;" v-if="auditData.transTypeName == '网上交易（挂牌）'">挂牌开始时间</td>
            <td style="width:140px;" v-else>拍卖开始时间</td>
            <td>{{auditData.beginFocusTime}}</td>
      ...
      <div style="margin-top:30px;">
        <el-table ref="preReviewTable" :data="licenseDatas" style="width: 100%" v-loading="loading" element-loading-text="数据加载中" border>
          ...
          <el-table-column prop="statusName" label="状态" min-width="130">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getAuditData(){
      ...
      this.loading = true;
      this.$get('/epf-trade/admin/translicense/beforeAuditList',{targetId: targetId}).then(res=>{
        if(res.code == 0){
    ...
    getLicenseDatas(targetId){
      this.loading = true;
      this.$get('/epf-trade/admin/translicense/getFrantApplication',{targetId:targetId}).then(res=>{
        if(res.code == 0){
    ...
    openAudit(row){
      //判断是否可以审核
      let endEarnestTime = new Date(this.auditData.endEarnestTime);
      let nowDate = new Date();
      if(endEarnestTime<nowDate){
          this.$message.error('保证金截止时间已到，不能再进行审核')
          return;
      }
      console.log("获取的row对象是：",row)
        let title = '审核页面';
        let routerPath = '/frontExamine?targetId='+this.targetId+'&id='+row.id+'&businessId='+row.businessId+'&taskId='+row.taskId+'&procInsId='+row.procInsId+'&procDefId='+row.procDefId;
        let tabObj = {title,routerPath}
        //将当前点击的选项添加到vuex中
        this.setTabsList(tabObj);
        console.log(row.procInsId+":审核")
        this.$router.push({path:'/frontExamine', query:{targetId: this.targetId, id:row.id ,businessId:row.businessId,taskId:row.taskId,procInsId:row.procInsId,procDefId:row.procDefId}});
    },
    //打开预审结果页面
    openAuditResult(row){
        let title = '审核结果';
        let routerPath = '/frontExamineResult?targetId='+this.targetId+'&id='+row.id+'&businessId='+row.businessId+'&taskId='+row.taskId+'&procInsId='+row.procInsId+'&procDefId='+row.procDefId;
        let tabObj = {title,routerPath}
        //将当前点击的选项添加到vuex中
        this.setTabsList(tabObj);
        console.log(row.procInsId+":审核结果")
        this.$router.push({path:'/frontExamineResult',query:{targetId:this.targetId,id:row.id,businessId:row.businessId,taskId:row.taskId,procInsId:row.procInsId,procDefId:row.procDefId}});
    },
  },
  watch: {
    $route(v1,v2){
