    ...
    <div class="filter_wrap">
      ...
      <el-tabs style="margin-top:35px;">
        ...
        >
          ...
          <el-table-column prop="recorderName" label="经办人" width="160"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getTransTargetList() {
      ...
      this.$get('/epf-trade/admin/transtarget/getTransTargetList',param).then(res=>{
        if (res.code == "0") {
    ...
    handleDelete(index, row) {
      let that = this;
      let url = "/epf-trade/admin/transtarget/deleteTransTarget";
    ...
    // 保存流拍原因--确定
    saveReason(){
      this.$post("/epf-trade/transtransactionresul/addReasonByTargetId", {targetId: this.currentObjId, context: this.flowpatreasonData.textarea}).then(res => {
        if (res.code != "0") {
          this.$message.error(res.msg); //失败
          return;
        }
    ...
    handleClick(item,oper) {
      ...
      if(oper==="bidding"){routerPath='landmarkInfoBidding',title='竞价记录'}
      var tabObj = {title,routerPath}
      this.setTabsList(tabObj)
