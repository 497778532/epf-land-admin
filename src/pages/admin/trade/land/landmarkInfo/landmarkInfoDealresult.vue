...
export default {
  ...
  methods:{
    ...
    init(targetId,token){
      ...
      let url = "/epf-trade/admin/transtarget/getSuccessfulTradingResultsByTargetId"; // 成交结果
      this.$get(url, {'targetId': targetId,'token':token}).then(res => {
        if (res.code != "0") {
