            ...
            <div class="publish_main_cont">
              <el-tabs>
                ...
                <el-table :data="landInfo1" style="width: 100%," border>
                  ...
                  <el-table-column prop="endTransTime" label="成交时间" width="300"></el-table-column>
            ...
            <div class="publish_main_cont">
              <el-tabs>
                <el-table ref="multipleTable" :data="contactInfo" style="width: 100%," border>
                  <el-table-column prop="orgName" label="联系单位" width="340"></el-table-column>
...
import qs from "qs";
export default {
  name: "publicityRelease",
  data() {
    let checkForm2=(rule, value, callback)=> {
      var len = 0;
      ...
      if ((len < 0 && len > 0) || len > 200) {
        //重点重点，下面就是填写提示的文字
        callback(new Error("您输入的内容过长，字数限制200个字以内 。(200个字时400个字符）"));
      } else {
        callback();
      }
    }
    ...
    return {
      ...
      rules:{
        publictTitle:[
          { required: true, message: "请输入公示标题" },
          // { validator: (rule, value, callback) =>{this.validateNum(rule, value, callback,0,50,'长度在50字符以内,(1个汉字是2个字符)')}  },
          // {
        ...
        publictContext:[
          { required: true, message: "请输入公示内容" },
          // { validator: (rule, value, callback) =>{this.validateNum(rule, value, callback,0,200,'长度在200字符以内,(1个汉字是2个字符)')}  },
        ]
      },
      id: ""
    };
  ...
  methods: {
    init(targetId) {
      let url = "/epf-trade/admin/transtarget/getAddTransResultsPublic"; // 成交结果
    ...
    publish(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$post(
            "/epf-trade/admin/transtarget/addTransResultsPublic",
            params
          ).then(res => {
            if (res.code != "0") {
              this.$message.error(res.msg); //失败
  ...
  mounted(){
    console.log("====landInfo===",(rule, value, callback) =>{this.validateNum(rule, value, callback,0,100,'长度在100字符以内（注意若是汉字，1个汉字是2个字符）')} )
  }
};
</script>
