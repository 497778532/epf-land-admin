              ...
              <ul class="publish_mainUl">
                ...
                <li>
                  <el-form-item class="unitContain" :label="'拟'+transGoods.circulationName+'价格：'">
                    <el-input :value="transGoods.price?Division(transGoods.price):0" size="small" disabled style="width:90%;"></el-input>
                ...
                <li>
                  <el-form-item label="是否连同地上建筑物、其他附着物：" required>
                ...
                <li v-if="transGoods.isAdjunct == '0'">
                  <el-form-item class="unitContain" label="建筑面积：">
                    <el-input v-model="transGoods.buildArea" size="small" disabled style="width:80%;"></el-input>
...
<!--
            ...
            <!-- <div class="publish_main_title">
              ...
              <div class="fold f-right" @click="swichFlot(5)">
                <div v-if="!foldshow5">
                  <span>展开</span>
            ...
            <el-table-column
              ...
              >
              <template slot-scope="scope">
                <span style="text-align:center;cursor:pointer;color: #4772d5;">{{ scope.row.publishNo }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="infoTitle"
              ...
              align="left">
              <template slot-scope="scope">
                <span class="titleHover" @click="rowClickHandle(scope.row)" style="text-align:center;cursor:pointer;">{{ scope.row.title }}</span>
              </template>
            </el-table-column>
            <el-table-column
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        east: [{ required: true, message: "请输入东至", trigger: "blur" }],
        west: [{ required: true, message: "请输入西至", trigger: "blur" }],
        south: [{ required: true, message: "请输入南至", trigger: "blur" }],
        north: [{ required: true, message: "请输入北至", trigger: "blur" }]
      },
  ...
  methods: {
    ...
    getItemData: async function(id) {
      const data = await this.$get("/epf-trade/admin/transgoods/getTransGoods",{id:id});
      if (!data) {
        return;
      }
      if(data.transGoods.circulationName == "转让"){
        this.publishLandtype = "剩余使用"
        this.ruleForm.circulation = "transfer";
        this.ruleForm.circulationName = "转让";
        this.disabledYear = true
      }else if(data.transGoods.circulationName == "出租"){
