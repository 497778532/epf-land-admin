                ...
                <ul class="publish_mainUl">
                  ...
                  <li>
                    <el-form-item label="公告类型：" prop="noticeType" required>
                      <el-select :disabled="disabled" v-model="ruleForm.noticeTypeName" placeholder="请选择" style="width:100%;"></el-select>
                    </el-form-item>
                  </li>
                   <li>
                  ...
                  >
                    ...
                    <!-- <el-table-column label="操作" width="240" align="center">
                      <template slot-scope="scope">
                        ...
                        <el-button @click="handleSubmit(scope.row)" type="text" size="small">提交</el-button>
                        <el-button
                          @click.native.prevent="deleteRow(scope.$index, landmarkInfo)"
                          type="text"
                          size="small"
                          class="deltext"
                        >删除</el-button>
                  ...
                  >
                    ...
                    <!-- <el-table-column label="操作" width="240" align="center">
                      <template slot-scope="scope">
                        ...
                        <el-button @click="handleSubmit(scope.row)" type="text" size="small">提交</el-button>
                        <el-button
                          @click.native.prevent="deleteRow(scope.$index, bankInfo)"
                          type="text"
                          size="small"
                          class="deltext"
                        >删除</el-button>
...
export default {
  name: "announcementInfoChangedetail",
  data() {
    return {
  ...
  created() {
    ...
    this.$get("/epf-trade/admin/transnotice/getTransNotice", { id: this.$route.query.id}).then( res => {
      if (res.code == "0") {
        ...
        //   console.log("新增公告附件模板返回---", JSON.parse(res.ztreeJson));
        //   for (let i = 0; i < resData.length; i++) {
        //     if (resData[i].code.indexOf("NOTICE") != -1) {
        //       // 公告附件清单
        //       this.tableData = resData[i].childData;
        //       // this.fileTemplateId = resData[i].code
        //       this.noticeCode = resData[i].code; //附件code赋值
        //     }
        //   }
        //   let files = this.tableData;
        //   this.ruleForm.pId = this.tableData[0].tid;
        //   console.log(
        //     "附件模板数据---",
        //     this.tableData,
        //     "附件tid",
        //     this.tableData[0].tid
        //   );
        //   // for (let index = 0; index < files.length; index++) {
        //   //   this.transGoods[files[index].tid] = [];
        //   // }
        // });
      }
    });
  ...
  methods: {
    ...
    setState(state){
      ...
      //改变状态
      this.$post("/epf-trade/admin/transnotice/revocationIssue",{"id":id, "status":publishStatus}).then(res => {
        if(res.code != '0'){
          //失败
    ...
    getAddTransBankList() {
      ...
      this.$get("/epf-trade/admin/transbank/getTransBankList", bankTest).then(
        res => {
          if (res.code == "0") {
