    ...
    <el-dialog title="标的信息" :visible.sync="landMarkDialogVisible" width="60%">
      <el-form
            ...
            <el-col :span="9">
              <el-form-item label="经办人：" prop="insertUser">
                ...
                <el-autocomplete style="width:100%;"
                  class="inline-input"
                  v-model="landmarkForm.insertUserValue"
        ...
        <!-- <el-row>
          ...
          <el-col :span="2" class="oper">
            <div class="reset" style="width:80%;" @click="resetlandmarkInfo()">重置</div>
          </el-col>
          <el-col :span="2" class="oper">
            <div class="primaryt" style="width:80%;" @click="conditionQuerylandmarkInfo()">查询</div>
          </el-col>
        </el-row> -->
      </el-form>
      <el-table
        ...
        <el-table-column width="200" align="center" label="挂牌/拍卖开始时间">
          <!-- <template slot="header" v-if="landmarkInfoData">
            <span v-if="landmarkInfoData[0].transTypeMark=='listing_mode'">挂牌开始时间</span>
            <span v-else>拍卖开始时间</span>
          </template> -->
          <!-- beginFocusTime-----挂牌开始时间-----listing_mode -->
          <!-- beginLimitTime-----限时竞价开始时间（拍卖开始时间）---auction_mode(拍卖) -->
          <template slot-scope="scope">
            <span v-if="scope.row.transTypeMark=='listing_mode'">{{scope.row.beginFocusTime}}</span>
            <span v-if="scope.row.transTypeMark=='auction_mode'">{{scope.row.beginLimitTime}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="recorderName" label="经办人" width="120" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getUsers(){
      ...
      this.$get(`/epf-admin/admin/user/getUsersByCurrentOrRole?${token}`, {}).then(
        res => {
          if (res.code == 0) {
            this.users = res.list;
    ...
    handleSelect(item) {
      this.landmarkForm.insertUserValue = item.value
      this.landmarkForm.insertUser = item.id
    },
    ...
    getAddTransTargetList(params) {
      ...
      this.$get(
        "/epf-trade/admin/transtarget/getTransTargetList",
        formTest
      ).then(res => {
        if (res.code == "0") {
    ...
    getAddTransBankList() {
      ...
      this.$get("/epf-trade/admin/transbank/getBankList", bankTest).then(
        res => {
          if (res.code == "0") {
    ...
    submitForm(formName) {
      ...
      if(str.length == 0){
        ...
        this.$refs[formName].validate(async valid => {
          ...
          this.$get(
              `/epf-trade/admin/transtarget/getTransTargetById?targetId=${this.landmarkInfo[0].id}`
          ).then(res => {
                  debugger
                  let _this = this;
                  //获取地块的信息
    ...
    conditionQuerylandmarkInfo() {
      console.log(!this.landmarkForm.insertUser)
      if(!this.landmarkForm.insertUser){
        this.$message.error("您输入得经办人不存在，请根据提示选择");
        return;
      }
      this.getAddTransTargetList(this.landmarkForm)
    },
    ...
    resetlandmarkInfo() {
      ...
      this.landmarkForm.insertUserValue = '';
      this.targetNo = "";
      this.getAddTransTargetList(this.landmarkForm);
    },
    //新增银行的列表确定
