              ...
              <div class="publish_main_cont">
                <div class="publish_main_cont_table">
                  ...
                  >
                    ...
                    <el-table-column prop="recorderName" label="经办人" align="center"></el-table-column>
            ...
            <el-col :span="12">
              <el-form-item label="公告开始时间：" prop="beginNoticeTime">
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.beginNoticeTime"
            ...
            <el-col :span="12">
              <el-form-item label="竞买申请开始时间：" prop="beginApplyTime">
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.beginApplyTime"
            ...
            <el-col :span="12">
              <el-form-item label="挂牌开始时间：" prop="beginFocusTime" >
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.beginFocusTime"
            ...
            <el-col :span="12">
              <el-form-item label="挂牌截止时间：" prop="endFocusTime">
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.endFocusTime"
            ...
            <el-col :span="12">
              <el-form-item label="保证金缴纳截止时间：" prop="endEarnestTime">
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.endEarnestTime"
            ...
            <el-col :span="12">
              <el-form-item label="限时竞价开始时间：" prop="beginLimitTime" v-if="isShow">
                <el-date-picker
                  @change="validate()"
                  :editable="false"
                  :disabled="sysTime >= targetTime.beginLimitTime"
...
<script>
    import editor from "../add/announcementeditor";
    ...
    export default {
        name: "announcementInfoChangeAmend",
        data() {

            //公告开始时间
            const beginNoticeTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.targetTime.beginApplyTime){
                        if(this.isShow){
                            //挂牌
                            if(this.targetTime.beginNoticeTime>=this.targetTime.beginApplyTime){
                                //公告开始时间要小于竞买申请开始时间
            ...
            const beginApplyTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.isShow){
                        //挂牌
                        if(this.targetTime.beginNoticeTime){
                            if(this.targetTime.beginNoticeTime>=this.targetTime.beginApplyTime){
                                //竞买申请开始时间不能小于公告开始时间
                        ...
                        if(this.targetTime.beginFocusTime){
                            if(this.targetTime.beginFocusTime < this.targetTime.beginApplyTime){
                                //竞买申请开始时间 不能大于 挂牌开始时间
                                callback(new Error("竞买申请开始时间不能大于挂牌开始时间"));
                            }
                        }
                        //和保证金截止对比  endEarnestTime
                        if(this.targetTime.endEarnestTime){
                            if(this.targetTime.endEarnestTime <= this.targetTime.beginApplyTime){
                        ...
                        //拍卖
                        if(this.targetTime.endEarnestTime){
                            if(this.targetTime.beginApplyTime>=this.targetTime.endEarnestTime){
                                //竞买申请开始时间要小于保证金缴纳截止时间
                                callback(new Error("竞买申请开始时间要小于保证金缴纳截止时间"));
                            }
                        }
                        if(this.targetTime.beginNoticeTime){
                            if(this.targetTime.beginApplyTime<=this.targetTime.beginNoticeTime){
                                //竞买申请开始时间不能小于公告开始时间
                                callback(new Error("竞买申请开始时间必须大于公告开始时间"));
                            }
                        }else{
            ...
            const beginFocusTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.isShow){
                        //挂牌
                        if(this.targetTime.beginApplyTime){
                            if(this.targetTime.beginApplyTime>this.targetTime.beginFocusTime){
                                //挂牌开始时间不能小于竞买申请开始时间
                        ...
                        if(this.targetTime.endFocusTime){
                            if(this.targetTime.endFocusTime<=this.targetTime.beginFocusTime){
                                //挂牌开始时间必须小于挂牌截止时间
            ...
            const endFocusTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.isShow){
                        ...
                        if(this.targetTime.endEarnestTime){
                            if(this.targetTime.endFocusTime<=this.targetTime.endEarnestTime){
                                //挂牌截止时间必须大于保证金截止时间
                        ...
                        if(this.targetTime.beginLimitTime){
                            if(this.targetTime.beginLimitTime<this.targetTime.endFocusTime){
                                //挂牌截止时间必须小于限时竞价开始时间
            ...
            const endEarnestTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.isShow){
                        //挂牌
                        if(this.targetTime.endFocusTime){
                            if(this.targetTime.endEarnestTime>=this.targetTime.endFocusTime){
                                //保证金截止时间必须小于挂牌截止时间
                        ...
                        //拍卖
                        if(this.targetTime.beginLimitTime){
                            if(this.targetTime.endEarnestTime>=this.targetTime.beginLimitTime){
                                //保证金缴纳截止时间要小于拍卖开始时间
            ...
            const beginLimitTimeVa = (rule, value, callback) => {
                setTimeout(() => {
                    if(this.isShow){
                        //挂牌
                        if(this.targetTime.endFocusTime){
                            if(this.targetTime.endFocusTime>=this.targetTime.beginLimitTime){
                                //限时竞价开始时间要大于挂牌截止时间
        ...
        created() {
            let that = this;
            if(that.$route.query.resubmitNotice){   //重新提交
              that.modelTye = "2";
            }else if(that.$route.query.edit){
              that.modelTye = "1"; // 修改变更公告
            }

            that.sysTime = new Date();
            if(that.$route.query.edit){
              //设置附件ID
              that.belong = that.$route.query.id;
              //设置附件ID的存储字段
              that.ruleForm.enclosureId = that.$route.query.id;
              that.ruleForm.id = that.$route.query.id;
            }else{
                this.$get("/epf-trade/api/createId", {}).then(res => {
                  that.ruleForm["id"] = res.id; //创建对象ID
                  //设置附件ID
                  that.belong = res.id;
                  //设置附件ID的存储字段
                  that.ruleForm.enclosureId = res.id

                  that.ruleForm.noticeId = that.$route.query.id;
                  that.areaCode = "";
              });
            }
            // this.setnoticeNo2();

            this.$get("/epf-trade/admin/transnotice/getTransNotice", {id: that.$route.query.id}).then(res => {
                if (res.code == "0") {
                  if(that.$route.query.edit){
                    debugger;
                    //如果是修改进来
                    that.ruleForm = res.map.map.transNotice;
                    //重新设置公告信息的值
                    that.info = res.map.map.transNotice.content;
                    console.log("编辑器文本对象", that.info)
                    // --公告编号：
                    let NO = that.ruleForm.noticeNo
                    // 贵自然资告字[2019]0002号
                    //截取出贵
                    that.ruleForm['noticeNo1'] = NO.substring(0, 1)
                    //截取出2019
                    that.ruleForm['noticeNo2'] = NO.substring(NO.indexOf('[') + 1, NO.indexOf(']'))
                    //截取出0002
                    that.ruleForm['noticeNo3'] = NO.substring(NO.indexOf(']') + 1, NO.indexOf('号'))
                  }else {
                    //如果是的新增进来
                    console.log(res);
                    ...
                    if(that.$route.query.resubmitNotice){   //重新提交
                      that.ruleForm.noticeId = res.map.map.transNotice.noticeId;
                    }else{
                      that.ruleForm.noticeId = that.$route.query.id;
                    }
                    // --公告标题：
                    that.ruleForm.title = ruleForm.title
                    // --公告日期
                    that.ruleForm.noticeDate = ruleForm.noticeDate
                  }
                }
            });
            that.getAddTransTargetList(that.$route.query.id); //新增标的信息列表请求
            that.getAddTransBankList(that.$route.query.id); //新增银行信息列表请求

        },
        methods: {
          onKeyUp(e, editor,flag){
            this.flagZhcn = !flag
            ...
            updateRow(index) {
                ...
                if(this.targetTime.beginNoticeTime){
                    beginNoticeTime = new Date(this.targetTime.beginNoticeTime);
                }
                if(this.targetTime.beginApplyTime){
                    beginApplyTime = new Date(this.targetTime.beginApplyTime);
                }
                if(this.targetTime.beginFocusTime){
                    beginFocusTime = new Date(this.targetTime.beginFocusTime);
                }
                if(this.targetTime.endFocusTime){
                    endFocusTime = new Date(this.targetTime.endFocusTime);
                }
                if(this.targetTime.endEarnestTime){
                    endEarnestTime = new Date(this.targetTime.endEarnestTime);
                }
                if(this.targetTime.beginLimitTime){
                    beginLimitTime = new Date(this.targetTime.beginLimitTime);
                }


                this.targetTime.beginNoticeTime=beginNoticeTime;
                this.targetTime.beginApplyTime=beginApplyTime;
                this.targetTime.beginFocusTime=beginFocusTime;
                this.targetTime.endFocusTime=endFocusTime;
                this.targetTime.endEarnestTime=endEarnestTime;
                this.targetTime.beginLimitTime=beginLimitTime;
            ...
            init(params) {
                this.$get("/epf-trade/admin/transnotice/getTransNoticeList", params).then(
                    res => {
                ...
                this.$get(
                    ...
                    if (res.code == "0") {
                        ...
                        for (let i = 0; i < this.landmarkInfoData.length; i++) {
                            ...
                            let type = 1;
                            if(this.modelTye == ""){   //新增,取生效的值
                              type = 3;
                            }else if(this.modelTye == "1" || this.modelTye == "2"){   //1 修改  2 重新提交，取公告对应的值
                              type = 2;
                            }
                            this.$get(
                                `/epf-trade/admin/transtarget/getTransTargetNewTime`,
                                {targetId: id, type: type ,noticId: this.ruleForm.id}
                            ).then(res => {
                              debugger
                              let targetEntity = res.targetEntity;
                              this.info = res.targetEntity.transCondition
                              if(this.info != null){
                                tinymce.activeEditor.setContent(this.info)
                              }
                              targetEntity["noticeId"] = this.ruleForm.id;
                              targetEntity["targetId"] = id;
                              this.targetTimeList.push(targetEntity);
                            });
                        }
                    } else {
            ...
            getAddTransBankList(id) {
                this.$get("/epf-trade/admin/transbank/getTransBankListByNoticeId", {id: id}).then(
                    res => {
                        if (res.code == "0") {
            ...
            submitForm(formName) {
              ...
              if(str.length == 0){
                ...
                this.$message.error("公告内容长度在2000字以内")
              }else{
                this.$refs[formName].validate(async valid => {
                    if (valid) {
                        ...
                        if(this.modelTye == "1"){
                          ...
                          this.$post('/epf-trade/admin/transnotice/checkTransNoticeNo', {
            ...
            saveNotice(type) {
                ...
                this.ruleForm.content = this.info;
                this.ruleFormSubmit = {
                ...
                if(this.modelTye == "1"){   //修改变更公告
                  let url =
                    "/epf-trade/admin/transnotice/editTransNotice?function=" + type; // 新增接口地址
                  this.$useAppend(url, {
                ...
                }else if (this.modelTye == "2") { // 变更公告重新提交
                  let url =
                      "/epf-trade/admin/transnotice/editTransNotice?function=" + type; // 新增接口地址
                  this.$useAppend(url, {
                      noticeJson: this.ruleFormSubmit
                    ...
                    if (res.code == 0){
                      ...
                      this.$get("/epf-trade/admin/transnotice/setResubmitNotice", {id: id}).then(res2 => {
                          if (res2.code == 0) {
                            this.$useAppend(url2, {
