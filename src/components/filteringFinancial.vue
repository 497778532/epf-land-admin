    ...
    <div class="switchHeight_box">
      ...
      <el-row :gutter="0">
        ...
        <el-col v-if="!closeSelect" :span="2">
          <span class="filter_title">标的状态 :</span>
        </el-col>
        <el-col v-if="!closeSelect" :span="3">
          <el-select
            ...
            placeholder="请选择标的状态"
            @change="goodSelect(dictionary.trans_status,filterData.status,$event)"
          >
            <el-option
...
export default {
  ...
  data() {
    return {
      ...
      switchHeight: 49,
      flexibleOpen: false,
      dicRequest: ["flow_mode", "delegation_status","trans_status"], //字典码请求
      dictionary: {
        //字典码结果
        purchasr_status: "",
        purchasr_auth: "",
  ...
  created() {
    ...
    Promise.all(reqArr).then(res => {
      ...
      for (let item in this.dictionary) {
        ...
        this.dicNum++;
        if(item=='trans_status'&&this.targeType==true){
          this.dictionary.trans_status = this.dictionary.trans_status.filter(item => (item.dictKey == 'trans_status-005'||item.dictKey == 'trans_status-006'||item.dictKey == 'trans_status-007'||item.dictKey == 'trans_status-008'))
        }
      }
      // trans_atatus_008---已公示
      // trans_atatus_005---成交待审核
      // trans_atatus_006---未成交（流拍）
      // trans_atatus_007---已成交
