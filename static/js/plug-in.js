(function($){
	...
	jQuery.Pop  = function(Html,Type,Callback,Title) {	
	var BoxId=id_();
	var PopType={
		alert: {
			Title: "提示",
		...
		confirm2:{
			Title: "注意",
			Btn:{
				yes:{vla:"是",class:"btn btn-primary",ope:function(){Close()}},
				no:{vla:"否",class:"btn btn-primary",ope:function(){if(Callback){Callback();Close()}else{Close()}}}
			}
		},};
	...
	var $BoxBg=$("<div>").css({
		...
		if(Callback){Callback();Close()}else{Close()}
		});		
	...
	jQuery.Pro  = function(Html,Type) {
		...
		/**
		 * 注释原因：图片会发生请求，造成控制台报错，项目用不上，先注释掉
		 */
